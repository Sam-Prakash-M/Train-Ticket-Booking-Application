...
<head>
    ...
    <link rel="stylesheet" href="payment.css?v=4">
</head>
<body>
...
<div class="page-wrapper">
    <div class="payment-card animate-slide">
        ...
        <div class="price-card glass animate-fade">
            <span class="price-title">Total Amount</span>
            <span class="price-value">₹ <%= String.format("%.2f", total) %></span>
        </div>

        <!-- Passenger Section -->
        <div class="section glass animate-fade-up">
            <h3><i class="icon-user"></i> Passenger Details</h3>
            <ul class="passenger-list">
                <% for(int i=0; i<names.length; i++) { %>
                    <li>
                        <span class="bullet"></span>
                        <strong><%= names[i] %></strong>  
                        <small>(Age: <%= ages[i] %>, <%= genders[i] %>, Berth: <%= berths[i] %>)</small>
                    </li>
                <% } %>
            </ul>
        </div>
        ...
        <div class="section glass animate-fade-up delay-1">
            <h3><i class="icon-bill"></i> Fare Breakdown</h3>
        ...
        <!-- Payment Buttons -->
        <h3 class="pay-title animate-fade-up delay-2">Select Payment Method</h3>

        <div class="gateways animate-fade-up delay-3">

            <button class="pay-btn razor" onclick="payWithRazorpay('<%= total %>')">
                <img src="https://upload.wikimedia.org/wikipedia/commons/8/89/Razorpay_logo.svg">
                <span>Razorpay</span>
            </button>

            <button class="pay-btn cashfree" onclick="payWithCashfree('<%= total %>')">
                 <img src="https://images.seeklogo.com/logo-png/47/1/cashfree-payments-logo-png_seeklogo-479373.png">
                <span>Cashfree</span>
            </button>
